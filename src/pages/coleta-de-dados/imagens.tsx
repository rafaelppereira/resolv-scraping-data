...
export function ScrapingImages() {
  const { toast } = useToast()
  const [uploadedFile, setUploadedFile] = useState(null)
  const [isLoading, setIsLoading] = useState(false)
  const [resultScraping, setResultScraping] = useState<{
    data: any
    extension: string
  } | null>(null)
  ...
  async function handleScrapingImage() {
    try {
      ...
      const { data } = await api.post('/scraping/image', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      })

      localStorage.setItem('scrapingImage', JSON.stringify(data))
  ...
  return (
    <>
      <Helmet title="Coleta de dados em imagens" />
